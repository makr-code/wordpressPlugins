<?php
/**
 * ThemisDB Pulse - Minimal Generic Functions
 *
...
define( 'THEMISDB_PULSE_VERSION', '3.3.0' );

// Backward-compatibility alias for legacy references.
if ( ! defined( 'THEMISDB_V3_VERSION' ) ) {
    define( 'THEMISDB_V3_VERSION', THEMISDB_PULSE_VERSION );
}

// Required plugin map: slug => main plugin file path.
define(
    'THEMISDB_PULSE_REQUIRED_PLUGINS',
    array(
...
// Backward-compatibility alias for legacy references.
if ( ! defined( 'THEMISDB_V3_REQUIRED_PLUGINS' ) ) {
    define( 'THEMISDB_V3_REQUIRED_PLUGINS', THEMISDB_PULSE_REQUIRED_PLUGINS );
}

define( 'THEMISDB_PULSE_TEXT_DOMAIN', 'themisdb-pulse' );

require_once get_template_directory() . '/includes/class-themisdb-v3-hero-context-nav.php';
...
function themisdb_v3_setup() {
    load_theme_textdomain( THEMISDB_PULSE_TEXT_DOMAIN, get_template_directory() . '/languages' );
    ...
    themisdb_v3_ensure_default_nav_menus();
}

/**
 * Make standard taxonomy filters available on pages so template sections can
 * select page-based content by category and tag.
 */
function themisdb_v3_register_page_taxonomies() {
...
/**
 * Ensure the theme always has registered menus assigned for block navigation output.
 */
function themisdb_v3_ensure_default_nav_menus() {
    ...
    foreach ( $default_locations as $location => $label ) {
...
function themisdb_v3_collect_missing_required_plugins() {
    ...
    $missing = array();
    foreach ( THEMISDB_PULSE_REQUIRED_PLUGINS as $slug => $plugin_file ) {
...
function themisdb_v3_render_missing_required_plugins_notice() {
    ...
    foreach ( $missing as $slug => $plugin_file ) {
        ...
        $action_link = sprintf(
            '<a href="%1$s">%2$s</a>',
            esc_url( $install_url ),
            esc_html__( 'Installieren/Aktivieren', THEMISDB_PULSE_TEXT_DOMAIN )
        );
        ...
        if ( file_exists( WP_PLUGIN_DIR . '/' . $plugin_file ) ) {
            ...
            $action_link = sprintf(
                ...
                esc_html__( 'Jetzt aktivieren', THEMISDB_PULSE_TEXT_DOMAIN )
            );
        }
    ...
    echo wp_kses_post(
        sprintf(
            ...
            __( 'ThemisDB Pulse benoetigt folgende Plugins fuer den vollen Funktionsumfang: %s', THEMISDB_PULSE_TEXT_DOMAIN ),
            implode( ', ', $items )
        )
    );
    echo '</p></div>';
...
/**
 * Resolve local context links for the current query context.
 *
 * @param int $limit Max number of links.
 * @return array<int,array<string,mixed>>
 */
function themisdb_v3_get_local_context_links( $limit = 8 ) {
    return ThemisDB_V3_Hero_Context_Nav::get_local_context_links( $limit );
}

/**
 * Render Azure-like context navigation below hero.
 *
 * @param array<string,mixed> $atts Shortcode attributes.
 * @return string
 */
function themisdb_v3_render_hero_context_nav_shortcode( $atts = array() ) {
    return ThemisDB_V3_Hero_Context_Nav::render_shortcode( $atts );
}

/**
 * Collect posts for frontpage card grids with category-first selection and
 * an optional priority tag that is used to fill the grid first.
 *
 * @param array<int,string> $post_types         Allowed post types.
 * @param int               $limit              Max cards to return.
 * @param array<int,string> $category_slugs      Category slugs that define the section.
 * @param array<int,string> $priority_tag_slugs  Optional tag slugs that should be preferred.
 * @param array<int,int>    $ids                Explicit IDs to use instead of taxonomy selection.
 * @param string             $orderby            Ordering key.
 * @param string             $order              Ordering direction.
 * @return array<int,WP_Post>
 */
function themisdb_v3_collect_frontpage_cards( array $post_types, $limit, array $category_slugs, array $priority_tag_slugs, array $ids = array(), $orderby = 'date', $order = 'DESC' ) {
    ...
    $allowed_orderby = array( 'date', 'modified', 'title', 'rand', 'menu_order' );
    ...
    $explicit_ids = array_values(
        array_filter(
            array_map( 'absint', array_map( 'trim', explode( ',', implode( ',', $ids ) ) ) )
        )
    );
    ...
    $category_slugs = array_values( array_filter( array_map( 'sanitize_title', $category_slugs ) ) );
    $priority_tag_slugs = array_values( array_filter( array_map( 'sanitize_title', $priority_tag_slugs ) ) );

    $selected_posts = array();
    $selected_ids   = array();

    $collect_posts = static function( array $extra_tax_queries, array $exclude_ids = array() ) use ( $query_args, $category_slugs, $limit ) {
        $args = $query_args;
        $args['posts_per_page'] = $limit;

        if ( ! empty( $exclude_ids ) ) {
            $args['post__not_in'] = array_values( array_unique( array_filter( array_map( 'absint', $exclude_ids ) ) ) );
        }
        ...
        foreach ( $extra_tax_queries as $extra_tax_query ) {
    ...
    if ( ! empty( $priority_tag_slugs ) ) {
        $priority_posts = $collect_posts(
            array(
                array(
                    'taxonomy' => 'post_tag',
                    'field'    => 'slug',
                    'terms'    => $priority_tag_slugs,
                ),
            )
        );
        ...
        foreach ( $priority_posts as $post ) {
            ...
            if ( count( $selected_posts ) >= $limit ) {
                return array_slice( $selected_posts, 0, $limit );
            }
        }
    }

    if ( count( $selected_posts ) < $limit ) {
        $remaining_posts = $collect_posts( array(), $selected_ids );
        foreach ( $remaining_posts as $post ) {
            if ( in_array( (int) $post->ID, $selected_ids, true ) ) {
                continue;
            }
            $selected_posts[] = $post;
            $selected_ids[]   = (int) $post->ID;
...
/**
 * Render feature cards using filtered WordPress content while keeping the
 * existing landing-page card design.
 *
 * Shortcode example:
 * [themisdb_v3_feature_cards limit="6" post_types="post,page" tag="hero"]
 *
 * @param array<string,mixed> $atts Shortcode attributes.
 * @return string
 */
function themisdb_v3_render_feature_cards_shortcode( $atts = array() ) {
    $atts = shortcode_atts(
        array(
            'limit'         => 6,
    ...
    $tones = array(
        array(
            'border' => 'tv3-card-top-border--blue',
            'icon'   => 'tv3-product-card__icon--blue',
            'glyph'  => '&#128240;',
        ),
        array(
            'border' => 'tv3-card-top-border--navy',
            'icon'   => 'tv3-product-card__icon--navy',
            'glyph'  => '&#129302;',
        ),
        array(
            'border' => 'tv3-card-top-border--green',
            'icon'   => 'tv3-product-card__icon--green',
            'glyph'  => '&#9889;',
        ),
        array(
            'border' => 'tv3-card-top-border--cyan',
            'icon'   => 'tv3-product-card__icon--cyan',
            'glyph'  => '&#128187;',
        ),
        array(
            'border' => 'tv3-card-top-border--amber',
            'icon'   => 'tv3-product-card__icon--amber',
            'glyph'  => '&#129513;',
        ),
        array(
            'border' => 'tv3-card-top-border--red',
            'icon'   => 'tv3-product-card__icon--red',
            'glyph'  => '&#128202;',
        ),
    );
    ...
    <div class="wp-block-group tv3-product-grid themis-v3-fade-in">
        <?php foreach ( $posts as $index => $post_item ) : ?>
            ...
            $source_text  = '' !== trim( (string) $raw_excerpt ) ? (string) $raw_excerpt : wp_strip_all_tags( (string) get_post_field( 'post_content', $post_id ) );
            $description  = wp_trim_words( $source_text, $words, '…' );
            $cta_label    = 'page' === get_post_type( $post_id ) ? __( 'Seite lesen →', 'themisdb-v3' ) : __( 'Beitrag lesen →', 'themisdb-v3' );
            ?>
            <div class="wp-block-group tv3-product-card">
                <div class="wp-block-group tv3-card-top-border <?php echo esc_attr( $tone['border'] ); ?>"></div>
                <p class="tv3-product-card__icon <?php echo esc_attr( $tone['icon'] ); ?>"><?php echo wp_kses_post( $tone['glyph'] ); ?></p>
                <div class="wp-block-group">
                    <h3 class="wp-block-heading tv3-product-card__title"><?php echo esc_html( $title ); ?></h3>
                    <p class="tv3-product-card__desc"><?php echo esc_html( $description ); ?></p>
                </div>
                <p class="tv3-product-card__cta"><a href="<?php echo esc_url( $permalink ); ?>"><?php echo esc_html( $cta_label ); ?></a></p>
            </div>
        <?php endforeach; ?>
    </div>
    <?php
 ...
 * @return string
 */
function themisdb_v3_render_docs_cards_shortcode( $atts = array() ) {
    $atts = shortcode_atts(
        array(
            'limit'         => 8,
            'post_types'    => 'post,page',
    ...
    <div class="tv3-docs-grid-cards">
        <?php foreach ( $posts as $post ) :
            ...
            $description = wp_trim_words( wp_strip_all_tags( $post->post_content ), $words, '...' );
...
function themisdb_v3_bind_navigation_ref_for_file_parts( $parsed_block, $source_block ) {
    ...
    if ( ! in_array( $location, array( 'primary', 'header_utility', 'docs', 'footer' ), true ) ) {
        return $parsed_block;
    }
...
function themisdb_v3_get_navigation_post_id_for_location( $location ) {
    ...
    $title = isset( $title_map[ $location ] ) ? $title_map[ $location ] : '';
    if ( '' === $title ) {
        return 0;
    }
    $found = get_posts(
...
function themisdb_v3_get_mixed_cards_state( array $atts = array(), array $request = array() ) {
    ...
    $supported_post_types = themisdb_v3_get_supported_content_post_types();
    ...
    $category_raw = isset( $raw_request['tv3_category'] ) ? (string) $raw_request['tv3_category'] : (string) ( $atts['category'] ?? '' );
    $category_slugs = array_values(
        array_filter(
            array_map(
                'sanitize_title',
                array_map( 'trim', explode( ',', $category_raw ) )
            )
        )
    );

    $post_types_raw = isset( $raw_request['tv3_post_types'] ) ? (string) $raw_request['tv3_post_types'] : (string) ( $atts['post_types'] ?? '' );
    $requested_post_types = array_values(
        array_filter(
            array_map(
                'sanitize_key',
                array_map( 'trim', explode( ',', $post_types_raw ) )
            )
        )
    );
    $scoped_post_types = ! empty( $requested_post_types ) ? array_values( array_intersect( $requested_post_types, $supported_post_types ) ) : $supported_post_types;

    if ( empty( $scoped_post_types ) ) {
        $scoped_post_types = $supported_post_types;
    }
...
function themisdb_v3_get_mixed_cards_url( array $state, array $overrides = array() ) {
    ...
    if ( ! empty( $args['category_slugs'] ) && is_array( $args['category_slugs'] ) ) {
        $url_args['tv3_category'] = implode( ',', array_map( 'sanitize_title', $args['category_slugs'] ) );
    }

    if ( ! empty( $args['post_types'] ) && is_array( $args['post_types'] ) ) {
        $url_args['tv3_post_types'] = implode( ',', array_map( 'sanitize_key', $args['post_types'] ) );
    }
...
function themisdb_v3_prepare_mixed_cards_data( array $state ) {
    $supported_post_types = themisdb_v3_get_supported_content_post_types();
    $requested_post_types = ! empty( $state['post_types'] ) && is_array( $state['post_types'] ) ? $state['post_types'] : array();
    $query_post_types     = ! empty( $requested_post_types ) ? array_values( array_intersect( $requested_post_types, $supported_post_types ) ) : $supported_post_types;

    if ( empty( $query_post_types ) ) {
        $query_post_types = $supported_post_types;
    }
    ...
    $query_args = themisdb_v3_apply_mixed_cards_context( $query_args, $state );

    if ( ! empty( $state['category_slugs'] ) && is_array( $state['category_slugs'] ) ) {
        $category_query = array(
            'taxonomy' => 'category',
            'field'    => 'slug',
            'terms'    => $state['category_slugs'],
        );
...
function themisdb_v3_render_mixed_cards_controls( array $data ) {
    ...
    <div class="tv3-card-toolbar-row">
        <form class="tv3-card-toolbar" method="get" action="<?php echo esc_url( themisdb_v3_get_mixed_cards_base_url() ); ?>" data-tv3-search-form>
            ...
            <input type="hidden" name="tv3_category" value="<?php echo esc_attr( implode( ',', array_map( 'sanitize_title', (array) ( $state['category_slugs'] ?? array() ) ) ) ); ?>" />
            <input type="hidden" name="tv3_post_types" value="<?php echo esc_attr( implode( ',', array_map( 'sanitize_key', (array) ( $state['post_types'] ?? array() ) ) ) ); ?>" />
...
function themisdb_v3_render_mixed_cards_block( array $attributes = array() ) {
    ...
    $atts = array(
        ...
        'post_types' => isset( $attributes['postType'] ) ? sanitize_text_field( (string) $attributes['postType'] ) : '',
    );
